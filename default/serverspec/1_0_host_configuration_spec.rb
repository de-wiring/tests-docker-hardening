...
describe '1 - Host Configuration', :if => cis_enabled?('1') do
  ...
  describe '1.1 - Create a separate partition for containers', :if => cis_enabled?('1.1') do
  
    describe file( cis_property('1.1',:var_lib_docker_path) ) do
      it { should exist }
  ...
  describe '1.2 - Use the updated Linux Kernel', :if => cis_enabled?('1.2') do
  ...
  describe '1.6 - Keep Docker up to date', :if => cis_enabled?('1.6') do
    describe command 'docker version' do
      its(:stdout) { should match /^Server version: #{cis_property('1.6',:docker_server_version)}/ }
    end
  end

  describe '1.7 - Only allow trusted users to control Docker daemon', :if => cis_enabled?('1.7') do
